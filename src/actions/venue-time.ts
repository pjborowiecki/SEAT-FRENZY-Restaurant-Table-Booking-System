...
import { venueTimeSchema } from "@/lib/validations/venue-time"
...
export type VenueTimeOpeningType = z.infer<typeof venueTimeSchema['shape']['openingTime']>
export type VenueTimeClosingType = z.infer<typeof venueTimeSchema['shape']['closingTime']>
...
export async function deleteVenueTimeAction(venueTimeId: number) {
  const result = await db.delete(venueTimes).where(eq(venueTimes.id, venueTimeId))
  return `Deleted venue opening times with id ${venueTimeId}`
}

export async function getVenueTimeAction(venueTimeId: number) {
  const result = await db.select().from(venueTimes).where(eq(venueTimes.id, venueTimeId))
  return JSON.stringify(result)
}

export async function getTimeForVenueAction(venueId: number, day: VenueTimeDayType) {
  const result = await db.select().from(venueTimes)
    .where(eq(venueTimes.venueId, venueId) && eq(venueTimes.day, day))
  return JSON.stringify(result)
}

export async function getAllTimesForVenueAction(venueId: number) {
  const result = await db.select().from(venueTimes).where(eq(venueTimes.venueId, venueId))
  return JSON.stringify(result)
}

export async function setOpeningTimeAction(venueId: number, day: VenueTimeDayType, newOpeningTime: VenueTimeOpeningType) {
  const validationResult = venueTimeSchema.shape.openingTime.safeParse(newOpeningTime)
  if (validationResult.success) {
    const result = await db.update(venueTimes)
...
export async function setClosingTimeAction(venueId: number, day: VenueTimeDayType, newClosingTime: VenueTimeClosingType) {
  const validationResult = venueTimeSchema.shape.closingTime.safeParse(newClosingTime)
  if (validationResult.success) {
    const result = await db.update(venueTimes)
