...
import { FANCINESS, PrismaClient } from "@prisma/client"
...
export default async function handler(
  ...
  const vivaanId =
    venues.find((venue) => venue.name === "Vivaan - fine Indian")?.id || 1
  const RamaKrishnaId =
    venues.find((venue) => venue.name === "RamaKrishna Indian")?.id || 1
  const coconutLagoonId =
    venues.find((venue) => venue.name === "Coconut Lagoon")?.id || 1
  const lastTrainToDelhiId =
    venues.find((venue) => venue.name === "Last Train to Delhi")?.id || 1
  const adrakYorkvilleId =
    venues.find((venue) => venue.name === "Adrak Yorkville")?.id || 1
  const curryishTavernId =
    venues.find((venue) => venue.name === "Curryish Tavern")?.id || 1
  const utsavId = venues.find((venue) => venue.name === "Utsav")?.id || 1
  const pukkaId = venues.find((venue) => venue.name === "Pukka")?.id || 1
  const kamasutraIndianId =
    venues.find((venue) => venue.name === "Kamasutra Indian")?.id || 1
  const eldoradoTacoId =
    venues.find((venue) => venue.name === "Eldorado Taco")?.id || 1
  const laBartolaId =
    venues.find((venue) => venue.name === "La Bartola")?.id || 1
  const elCatrinId = venues.find((venue) => venue.name === "El Catrin")?.id || 1
  const mariachisId =
    venues.find((venue) => venue.name === "3 Mariachis")?.id || 1
  const canoRestaurantId =
    venues.find((venue) => venue.name === "Cano Restaurant")?.id || 1
  const bluRistoranteId =
    venues.find((venue) => venue.name === "Blu Ristorante")?.id || 1
  const stelvioId = venues.find((venue) => venue.name === "Stelvio")?.id || 1
  const sofiaId = venues.find((venue) => venue.name === "Sofia")?.id || 1
